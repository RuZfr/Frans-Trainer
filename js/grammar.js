(() => {
  ...
  async function loadData(){
    if(data)return data;
    try{
      const url=new URL(`./data/grammar/cod.json?v=1.18.0`,document.baseURI);
  ...
  function makeQueue(selectedMode){
    if(selectedMode==="recognition") return shuffle(data.recognition).slice(0,Math.min(SESSION_LENGTH,data.recognition.length)).map(item=>({...item,type:"recognition"}));
    if(selectedMode==="advancedRecognition") return shuffle(data.advancedRecognition).slice(0,Math.min(SESSION_LENGTH,data.advancedRecognition.length)).map(item=>({...item,type:"recognition",advanced:true}));
    if(selectedMode==="replacement") return shuffle(data.replacement).slice(0,Math.min(SESSION_LENGTH,data.replacement.length)).map(item=>({...item,type:"replacement"}));
    if(selectedMode==="advancedReplacement") return shuffle(data.advancedReplacement).slice(0,Math.min(SESSION_LENGTH,data.advancedReplacement.length)).map(item=>({...item,type:"replacement",advanced:true}));
    const pool=[
      ...shuffle(data.recognition).slice(0,2).map(item=>({...item,type:"recognition"})),
      ...shuffle(data.advancedRecognition).slice(0,3).map(item=>({...item,type:"recognition",advanced:true})),
      ...shuffle(data.replacement).slice(0,2).map(item=>({...item,type:"replacement"})),
      ...shuffle(data.advancedReplacement).slice(0,3).map(item=>({...item,type:"replacement",advanced:true}))
    ];
    return shuffle(pool);
  }
